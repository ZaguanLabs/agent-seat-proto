...
const REVISION_TEXT: &str = "8";
const _: () = assert!(PROTOCOL_REVISION == 8);
...
impl<'a> Advertisement<'a> {
    /// Parses the exact revision-8 advertisement grammar without allocation.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn malformed_and_noncanonical_values_are_refused() {
        for value in [
            ...
            "agent-seat\x006",
            "agent-seat\x007",
            "agent-seat\x0007\0/tmp/s",
            "agent-seat\x00+7\0/tmp/s",
            "agent-seat\x007\0relative",
            "agent-seat\x007\0/tmp/s\0extra",
        ] {
