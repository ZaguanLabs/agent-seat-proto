...
| 0.1.7 / revision 9 | 0.1.11 | `agent-seat-x11` 0.1.31 / experimental Tier 0.5 input v3, obscured capture v2, and text transfer v1; Settings 0.1.10 | Linux, released Openbox 3.6.1 / isolated Xvfb with XTEST, XKB, X-Resource 1.2, and real X11 selection requestors; live Mageia/Openbox/Brave Beta/Suno; dual-era MCP process boundary | A focused same-client X11 requestor negotiates `TARGETS`, receives exact accented multiline UTF-8 twice across the bounded post-delivery service window, and retains every hostile refusal and cleanup property from 0.1.30. Live Brave/Suno evidence shows 0.1.30 reporting delivery without insertion, then 0.1.31 visibly inserting exact ASCII and accented multiline text. Companion 0.1.11 invalidates dead provider contexts without replay, requires fresh status/observation, and no longer leaks raw broken-pipe text. The separate grant visibly warns that the old clipboard cannot be restored and a manager may retain text. Delivery still does not generally prove application insertion, and physical-user priority remains unsupported. Legacy publishes 23 closed tools and modern 24 including `seat_release`. |
