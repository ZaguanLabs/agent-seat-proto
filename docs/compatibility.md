...
| 0.1.7 / revision 9 | 0.1.10 | `agent-seat-x11` 0.1.30 / experimental Tier 0.5 input v3, obscured capture v2, and text transfer v1; Settings 0.1.10 | Linux, released Openbox 3.6.1 / isolated Xvfb with XTEST, XKB, X-Resource 1.2, and real X11 selection requestors; dual-era MCP process boundary | A focused same-client X11 requestor negotiates `TARGETS` and receives exact accented multiline UTF-8. A different X client is refused without text; a prior owner is displaced without being read; selection loss preserves the later owner; an unresponsive target returns `offered` and cleans up. The separate grant visibly warns that the old clipboard cannot be restored and a manager may retain text. Delivery does not prove application insertion, real browsers/toolkits remain unverified, and physical-user priority remains unsupported. Legacy publishes 23 closed tools and modern 24 including `seat_release`. |
